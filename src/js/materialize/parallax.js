...
export const Parallax = function($, Component) {
  'use strict';
  ...
  class Parallax extends Component {
    constructor(el, options) {
      ...
      this.$img = !!Object.keys(this.$el.find('img').first()).length ? this.$el.find('img').first() : this.$el.find('video').first();
    ...
    _setupEventHandlers() {
      ...
      this.$img[0].addEventListener('load', this._handleImageLoadBound, { passive: true });

      if (Parallax._parallaxes.length === 0) {
        Parallax._handleScrollThrottled = M.throttle(Parallax._handleScroll, 5);
        window.addEventListener('scroll', Parallax._handleScrollThrottled, { passive: true });
    ...
    _updateParallax() {
      ...
      if (!this._enabled) {
        ...
        this.$img[0].style.transform = `translate3D(0, ${parallax - 70}px, 0)`;
      }
    }
  }
